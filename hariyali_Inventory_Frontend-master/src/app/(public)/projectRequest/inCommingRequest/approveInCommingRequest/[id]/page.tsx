...
import {
  GetRemainingQuantityByProjectStock,
  GetPriceListByProjectStock,
} from "@/src/services/apiService/stock/openingStock/openingStockServices";
import { GetFarmerGroupByProject } from "@/src/services/apiService/layerSetup/farmerGroup/farmerGroupServices";
import {
  useFieldArray,
  useForm,
  Controller,
...
function ApproveIncommingRequestFromGroup() {
  ...
  const {
    ...
    queryFn: () => GetCompairdItemsByGroup(parseInt(id)),
    enabled: !!id,
  });
  ...
  const {
    ...
    queryKey: ["project"],
    queryFn: GetFarmerGroupByProject,
  });

  const groupOptions = groupList?.map((item: FarmerGroup, index: number) => {
    return (
  ...
  const categoryQueries = useQueries({
    queries: typeIds?.map((typeId) => ({
      ...
      queryFn: () => GetCategory(parseInt(typeId)),
  ...
  const subCategoryQueries = useQueries({
    queries: categoryIds?.map((categoryId) => {
      return {
        ...
        queryFn: () => GetSubCategory(parseInt(categoryId)),
        enabled: !!categoryId,
      };
    }),
  });
  ...
  const itemsQueries = useQueries({
    queries: subCategoryIds?.map((subCategoryId) => {
      return {
        ...
        queryFn: () => GetItem(parseInt(subCategoryId)),
  ...
  const priceQueries = useQueries({
    queries: itemIds?.map((itemId) => {
      return {
        ...
        queryFn: () => GetPriceListByProjectStock(parseInt(itemId)),
        enabled: !!itemId,
      };
    }),
  });
  ...
  const remainingQuantityQueries = useQueries({
    queries: itemIdAndRate?.map(({ itemId, rate }, index) => {
      return {
        ...
        queryFn: () =>
          GetRemainingQuantityByProjectStock(parseInt(itemId), parseInt(rate)),
        enabled: !!itemId && !!rate,
      };
    }),
  });
  ...
  const {
    data: groupId,
    isError: isErrorGroupId,
    isLoading: isLoadingGroupId,
  } = useQuery({
    queryKey: ["groupIdByRequestId", id],
    queryFn: () => GetGroupIdByRequestId(parseInt(id)),
    enabled: !!id,
  });
  ...
  const onSubmit = async (data: FieldValues) => {
    try {
      ...
      const { status, message } = await CreateGivenToGroup(data);
      if (status) {
        router.push(`/inventory/givenToGroup`);
        ...
        <form onSubmit={handleSubmit((data) => onSubmit(data))}>
          ...
          <div className={"flex items-center gap-4 justify-end"}>
            <button
              ...
              onClick={() => router.push("/projectRequest/inCommingRequest")}
            >
              Back
            </button>
            <button
  ...
  if (isError || isErrorCompaired || isErrorGroupId) return <Error />;

  return (
    <>
      {GroupLoading || isLoadingCompaired || isLoadingGroupId ? (
        <Loader />
      ) : (
        mainForm
      )}
...
export default ApproveIncommingRequestFromGroup;
