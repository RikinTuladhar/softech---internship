...
import {
  GetTypeAccordingToGroupStock,
  GetCategoryAccordingToGroupStock,
  GetSubCategoryAccordingToGroupStock,
  GetItemAccordingToGroupStock,
} from "@/src/services/apiService/layerSetup/farmerGroup/farmerGroupServices";
import {

  GetRemainingQuantityByGroupStock,
  GetPriceListByGroupStock,
} from "@/src/services/apiService/stock/openingStock/openingStockServices";
import { GetFarmerAccordingToGroup } from "@/src/services/apiService/layerSetup/farmer/farmerServices";
import {
  useFieldArray,
  useForm,
  Controller,
...
import { CreateGivenToFarmer } from "@/src/services/apiService/request/requestServices";

function GiveToFarmerComp() {
  ...
  const farmerOptions = farmerList?.map((item: Farmer, index: number) => {
    return (
  ...
  const {
    ...
    queryFn: GetTypeAccordingToGroupStock,
  });
  ...
  const categoryQueries = useQueries({
    queries: typeIds?.map((typeId) => ({
      ...
      queryFn: () => GetCategoryAccordingToGroupStock(parseInt(typeId)),
  ...
  const subCategoryQueries = useQueries({
    queries: categoryIds?.map((categoryId) => {
      return {
        ...
        queryFn: () =>
          GetSubCategoryAccordingToGroupStock(parseInt(categoryId)),
        enabled: !!categoryId,
      };
    }),
  });
  ...
  const itemsQueries = useQueries({
    queries: subCategoryIds?.map((subCategoryId) => {
      return {
        ...
        queryFn: () => GetItemAccordingToGroupStock(parseInt(subCategoryId)),
  ...
  const priceQueries = useQueries({
    queries: itemIds?.map((itemId) => {
      return {
        ...
        queryFn: () => GetPriceListByGroupStock(parseInt(itemId)),
        enabled: !!itemId,
      };
    }),
  });
  ...
  const remainingQuantityQueries = useQueries({
    queries: itemIdAndRate?.map(({ itemId, rate }, index) => {
      return {
        ...
        queryFn: () =>
          GetRemainingQuantityByGroupStock(parseInt(itemId), parseInt(rate)),
        enabled: !!itemId && !!rate,
      };
    }),
  });
  ...
  const onSubmit = async (data: FieldValues) => {
    try {
      ...
      const { status, message } = await CreateGivenToFarmer(data);
      if (status) {
        router.push(`/inventory/givenToFarmer`);
  ...
  return <>{GroupLoading ? <Loader /> : mainForm}</>;
}

export default GiveToFarmerComp;
