...
import { CreateRequest } from "@/src/services/apiService/request/requestServices";
import { Me } from "@/src/services/apiService/setup/user/userServices";
import { getFarmerGroupById } from "@/src/services/apiService/layerSetup/farmerGroup/farmerGroupServices";
import {
  useFieldArray,
  useForm,
  Controller,
...
function AddRequestForm() {
  ...
  const {
    ...
    isError: farmerGroupDataError,
    isLoading: farmerGroupDataLoading,
  } = useQuery({
    queryKey: ["farmerGroupById", meData],
    queryFn: () => getFarmerGroupById(meData?.id),
    enabled: !!meData?.id && meData?.role === "farmerGroup",
  });
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    data = {
      ...
      projectId: farmerGroupData?.projectId,
      groupId: farmerGroupData?.id,
          ...
          <div className={"grid grid-cols-3 gap-3"}>
            <div>
              ...
              <input
                ...
                readOnly
                value={farmerGroupData?.project?.name}
              />
            </div>
            {meData?.role === "farmerGroup" && (
              <div>
                <label htmlFor="">
                ...
                <input
                  ...
                  className="inputStyle"
                  readOnly
                  value={farmerGroupData?.groupName}
                />
              </div>
            )}
            <div>
