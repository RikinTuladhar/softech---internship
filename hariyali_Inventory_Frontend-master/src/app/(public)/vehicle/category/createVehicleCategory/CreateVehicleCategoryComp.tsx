...
import { CreateVehicleCategory } from "@/src/services/apiService/vehicle/vehicleService";

export default function CreateVehicleCategoryComp({
  clickedDataId,
}: {
  ...
  const onSubmit = async (data: FieldValues) => {
    try {
      const { status } = await CreateVehicleCategory(data);
      if (status === 200) {
